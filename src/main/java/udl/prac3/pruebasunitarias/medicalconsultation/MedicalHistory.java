...
public class MedicalHistory {
    ...
    public MedicalHistory(HealthCardID cip, int memberShipNum)
        ...
        if (cip == null || memberShipNum <= 0) throw new IncorrectParametersException("CIP cannot be null and membership number must be positive");
    ...
    public void addMedicalHistoryAnnotations(String annot) {
        if (annot == null || annot.isBlank()) return;

        if (!history.isEmpty()) history += System.lineSeparator();

        history += annot;
    }

    public void setNewDoctor(int mshN) throws IncorrectParametersException {
        if (mshN <= 0) throw new IncorrectParametersException("Membership number must be positive");
